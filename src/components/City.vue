<template>
  <div class="city">
    ...
    >
      <ul>
        <li v-for="item in city" class="list-group" ref="listGroup">
          <h2 class="list-group-title">{{item.name}}</h2>
          <ul>
            <li class="list-group-item" v-for="city in item.cities">{{city.name}}</li>
          </ul>
        </li>
      </ul>
      ...
      >
        <ul>
          <li v-for="(item,index) in shortcut" class="item"
              :data-index="index"
  ...
  export default {
    ...
    computed: {
      ...
      fixedTitle() {
        ...
        return this.city[this.currentIndex] ? this.city[this.currentIndex].name : ''
      }
    },
    methods: {
